...
{
  public class IDealAppointAccountInterface 
    {
        public JObject AppointAccountList(int userId, string token)
        {
            JObject jobject = new JObject();
            string resultJson = HttpHelper.postJsonOne(Urls.APPOINT_ACCOUNT, userId, token);
            Console.Write("指定账户列表 返回结果：" + resultJson);
