...
{
    public class IUnitListAccountInterface
    {
        public JObject UnitListAccount(string token)
        {
            JObject jobject = new JObject();
    
            string resultJson = HttpHelper.postJsonOne(Urls.ACCOUNT_UNIT_LIST, 1, token);
            Console.Write("主账户所在的单元列表返回结果：" + resultJson);
