...
{
    public class IDelUnitIterface
    {
        public JObject DelUnit(int? id, string code, string token)
        {
            JObject jobject = new JObject();
            Dictionary<string, Object> map = new Dictionary<string, object>();
            map.Add("id", id);
            map.Add("code", code);
            string resultJson = HttpHelper.postJson(Urls.DEL_UNIT, map, token);
            Console.Write("删除单元返回结果：" + resultJson);
