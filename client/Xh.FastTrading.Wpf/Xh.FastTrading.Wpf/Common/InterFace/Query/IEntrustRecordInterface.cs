...
{
   public class IEntrustRecordInterface
    {
        public JObject EntrustRecord(int unit_id, string from, string to,string token)
        {
            ...
            map.Add("from", from);
            map.Add("to", to);
            string resultJson = HttpHelper.postJson(Urls.ENTRUST_RECORD_LIST, map, token);
            Console.Write("委托记录列表返回结果：" + resultJson);
