...
{
   public class IModifyAccountInterface
    {
            public JObject ModifyAccount(int id,string server_ip, int server_port, string code,
                string name,string full_name, string remarks, string limit_no_buying,
                double ratio_commission,double limit_ratio_single,double limit_ratio_gem_single, double limit_ratio_gem_total, 
                double ratio_capital_warning, decimal capital_initial,decimal capital_inferior, long capital_priority, 
                double capital_raobiao,decimal capital_raobiao_rate,string token)
            {
                ...
                Console.Write("修改主账户列表返回结果：" + resultJson);
