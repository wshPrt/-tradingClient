...
{
   public class ILimitTradingAccountInterface
    {
        public JObject LimitTrading(int id,int status,string token)
        {
            JObject jobject = new JObject();
            Dictionary<string, Object> map = new Dictionary<string, object>();
            map.Add("id", id);
            map.Add("status", status);
            string resultJson = HttpHelper.postJson(Urls.ACCOUNT_LIMIT_TRANSACTION, map, token);
            Console.Write("限制交易返回结果：" + resultJson);
