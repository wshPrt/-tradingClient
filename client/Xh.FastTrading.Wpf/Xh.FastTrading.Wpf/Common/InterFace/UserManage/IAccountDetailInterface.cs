...
{
   public class IAccountDetailInterface
    {
        public JObject AccountDetail(string token)
        {
            JObject jobject = new JObject();
            string resultJson = HttpHelper.postJson(Urls.ACCOUNT_LIST, null, token);
            Console.Write("主账户列表返回结果：" + resultJson);
