...
{
  public  class ICancelTradeInterface
    {
        public JObject CancelTrade(int id, int status, string token)
        {
            JObject jobject = new JObject();
            Dictionary<string, Object> map = new Dictionary<string, object>();
            map.Add("id", id);
            map.Add("status", status);
            string resultJson = HttpHelper.postJson(Urls.LIMIT_TRANSACTION, map, token);
            Console.Write("取消限制返回结果：" + resultJson);
