...
{
   public  class IOpenClosingPauseResumeInterface
    {
        public JObject OpenClosingPauseResume(int request,string token)
        {
            JObject jobject = new JObject();
            string resultJson = HttpHelper.postJsonOne(Urls.PAUSE_RESUME, request, token);
            Console.Write("开盘收盘暂停恢复返回结果：" + resultJson);
