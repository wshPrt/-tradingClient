...
{
  public class IOpenClosingInterface
    {
        public JObject OpenClosing(string token)
        {
            JObject jobject = new JObject();
            string resultJson = HttpHelper.postJson(Urls.OPEN_CLOSE_LIST, null, token);
            Console.Write("开盘收盘列表返回结果：" + resultJson);
